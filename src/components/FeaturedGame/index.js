...
export default function FeaturedGame() {
  ...
  const getGameList = async () => {
    try {
      let response = await axios.get("https://bwamern-storegg-backend.herokuapp.com/api/v1/players/landing-page");
    ...
    <section className="featured-game pt-50 pb-50">
      <div className="container-fluid">
        ...
        <div className="d-flex flex-row flex-lg-wrap overflow-setting justify-content-lg-between gap-lg-3 gap-4" data-aos="fade-up">
          {gameList.map((item) => {
            return (
              <div className="featured-game-card position-relative" key={item._id}>
                <Link to={`/detail-page/${item._id}`}>
                  <div className="blur-sharp">
                    <img src={`https://bwamern-storegg-backend.herokuapp.com/uploads/${item.thumbnail}`} width="205" height="270" alt="" />
                  </div>
                  <div className="cover position-absolute bottom-0 m-32">
                    <div className="d-flex flex-column h-100 justify-content-between text-decoration-none">
                      ...
                      <div>
                        <p className="fw-semibold text-white text-xl m-0">{item.name}</p>
